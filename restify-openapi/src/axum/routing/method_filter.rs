...
impl From<MethodFilter> for axum::routing::MethodFilter {
  fn from(value: MethodFilter) -> Self {
    // SAFETY: This transmute is safe because `MethodFilter` and `axum::routing::MethodFilter`
...
mod tests {
  ...
  fn from_http_method() {
    ...
    assert!(MethodFilter::try_from(Method::CONNECT)
      .unwrap_err()
      .to_string()
      .contains("CONNECT"));
  }
